class Admin::ModelBrowserCatalog
  ...
  ENTRIES = [
    ...
    Entry.new("document_file_google_drive_preview_uploads", "Google Driveプレビューアップロード", "Google Drive viewer 用に一時アップロードした文書ファイルです。", DocumentFileGoogleDrivePreviewUpload, %i[public_id document_file_id fingerprint drive_file_id expires_at deleted_at], nil),
    Entry.new("document_file_microsoft_graph_preview_uploads", "Microsoft Graphプレビューアップロード", "Microsoft Graph viewer 用に一時アップロードした文書ファイルです。", DocumentFileMicrosoftGraphPreviewUpload, %i[public_id document_file_id microsoft_graph_connection_id fingerprint drive_item_id expires_at deleted_at], nil),
    Entry.new("recurring_job_schedules", "定期ジョブ", "定期実行ジョブの設定と予実です。", RecurringJobSchedule, %i[public_id job_key job_class enabled interval_seconds next_run_at last_status], :admin_recurring_job_schedules_path),
    Entry.new("recurring_job_runs", "定期ジョブ実行履歴", "定期実行ジョブの実行履歴です。", RecurringJobRun, %i[public_id job_key status scheduled_at started_at finished_at], nil),
