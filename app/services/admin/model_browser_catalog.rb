class Admin::ModelBrowserCatalog
  ...
  ENTRIES = [
    ...
    Entry.new("document_sets", :document_permission, "文書セット", "用途別にまとめた文書の組み合わせです。", DocumentSet, %i[public_id name internal_only updated_at], :admin_document_sets_path),
    Entry.new("document_catalogs", :document_permission, "文書カタログ", "案件配下の文書一覧です。", DocumentCatalog, %i[public_id name internal_only updated_at], nil),
    Entry.new("consent_terms", :document_permission, "同意文面", "利用前同意のマスタです。", ConsentTerm, %i[public_id title term_kind version active updated_at], :admin_consent_terms_path),
    Entry.new("project_consent_settings", :document_permission, "案件同意設定", "案件ごとの同意要件を管理します。", ProjectConsentSetting, %i[project_id consent_term_id required_on_first_view required_on_download updated_at], :admin_project_consent_settings_path),
    ...
    Entry.new("git_import_sources", :import_sync, "Git連携", "同期元の定義です。", GitImportSource, %i[public_id name repository_url branch active updated_at], :admin_git_import_sources_path),
    ...
    Entry.new("webhook_deliveries", :external_integration, "Webhook配信", "Webhook送信履歴です。", WebhookDelivery, %i[public_id status response_status delivered_at updated_at], nil)
  ].freeze
