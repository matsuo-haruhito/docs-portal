...
| `rails_table_preferences` | `import { RailsTablePreferencesController } from "rails_table_preferences"` → `rails-table-preferences` として register | `rails_table_preferences` package root alias のみ（`/controller` direct alias は削除済み） | `docs/javascript_entrypoints.md` が package root named export を案内 | package-root import のみ使用。direct path alias は不要として削除 | screen issue では package-root import を基準にする |
