...
current `main` の `.github/workflows/build-docs.yml` を正本として、`test` `seed-smoke` `security-audit` `build-docs` の見分け方、`publish/manifest/publish.json` の生成、`docs-site.tar.gz` artifact、Rails import API 呼び出し条件、import failure 時の replay / rebuild 判断を追いやすくします。
...
current workflow 名は `ci` で、docs build 専用 workflow が別にあるわけではありません。GitHub Actions の画面や PR の Checks では `ci` という workflow run を開き、repo 内の定義は `.github/workflows/build-docs.yml` を確認します。

- `test`: `push` (`dev`, `main`) と `pull_request` で走る
- `seed-smoke`: `push` (`dev`, `main`) と `pull_request` で走る
- `security-audit`: `test` 成功後に走るが、対象は `main` への push と `dependabot[bot]` の pull request だけ
- `build-docs`: `test` 成功後に走るが、対象は `main` への push と `dependabot[bot]` の pull request だけ

つまり current 実装では、通常の human PR では `security-audit` と `build-docs` job は走りません。PR で docs build 自体を確認したいときは、この前提を踏まえて `test` / `seed-smoke` と source diff を見ます。

PR review / merge 判断では、PR body や過去コメントに書かれた CI 状態より、最新 head SHA の workflow run と job 一覧を優先して確認します。combined status の一覧が空でも、GitHub Actions 側に `ci` workflow run が存在することがあります。その場合は `ci` run の jobs を開き、`test` / `seed-smoke` の conclusion と、`security-audit` / `build-docs` が skipped なのか failure なのかを分けて読みます。

PR branch refresh や follow-up commit の後は、過去コメントの green CI を current head の evidence として扱いません。次の順で、見ている evidence が最新 head に対応しているかを確認します。

1. PR metadata の `head_sha` と compare の `ahead_by` / `behind_by` / `status` を控える
2. GitHub Actions の `ci` workflow run が同じ `head_sha` を指しているか確認する
3. job conclusion が `test` / `seed-smoke` / `security-audit` / `build-docs` のどれに対応しているかを分けて読む
4. PR body や過去コメントに書かれた success が別の head SHA なら、current head の green evidence とは扱わない
5. visual evidence、manual smoke、browser evidence など CI 以外の blocker が残っている場合は、CI evidence とは別項目で記録する

latest head に workflow run が未検出の場合は、merge 判断を完了させず、次のどれが必要かを PR の状況に合わせて選びます。

- maintainer に current head の CI evidence が不足していることを戻す
- GitHub Actions 側で再実行できるなら、対象 head の `ci` run を再実行して結果を見る
- branch policy や権限上再実行できない場合は、小さな follow-up commit で workflow を起こす必要があるかを判断する
- visual evidence blocker が主因なら、CI 再取得とは別に browser-capable evidence の有無を明記する

`skipped` は「green と同じ確認済み」ではありませんが、通常 human PR で条件により走らない job なら failure とも扱いません。どの job が条件で skipped になり得るかは、この section の job 条件と `.github/workflows/build-docs.yml` の `if:` を見比べます。

## 2. 最初の切り分け順

1. GitHub Actions の run で、止まっている job が `test` `seed-smoke` `security-audit` `build-docs` のどれかを確認する
2. `test` で止まっていれば、Rails app 本体や spec failure を先に見る
3. `seed-smoke` で止まっていれば、`.env.example`、DB 準備、seed 前提、runtime note を見る
4. `security-audit` で止まっていれば、Ruby、root JavaScript、Docusaurus のどの dependency tree の audit step で止まったかを先に見る
5. `build-docs` で止まっていれば、Docusaurus build、manifest 生成、artifact upload、import API 呼び出しのどこで止まったかを切り分ける
6. `security-audit` や `build-docs` 自体が run に出ていない場合は、event が `main` push ではないか、Dependabot PR ではない可能性を先に疑う
...
- まず失敗 step が `Audit Ruby dependencies`、`Audit JavaScript dependencies`、`Audit Docusaurus dependencies` のどれかを確認します
- `Audit Ruby dependencies` なら `Gemfile.lock` と Bundler / Rails 側の依存更新を見ます
- `Audit JavaScript dependencies` なら root の `package.json` / `package-lock.json` と app frontend / Vite 側の依存更新を見ます
- `Audit Docusaurus dependencies` なら `docusaurus/package.json` / `docusaurus/package-lock.json` と Docusaurus 側の package tree を見ます
- Rails gem bump PR で表面化していても、失敗 step が `Audit Docusaurus dependencies` なら Rails / Bundler 更新とは別論点として扱い、#1365 のような quality / security issue または別 PR に切り出します
- `npm audit fix --force` が breaking upgrade や major upgrade を示す場合は無条件に適用せず、Planner / maintainer に判断を戻します
...
- workflow 定義の正本は `.github/workflows/build-docs.yml`、Actions 上の workflow 名は `ci` である
- PR review / merge 判断では、PR body の過去記述より最新 head SHA の `ci` workflow run と job conclusion を優先する
- branch refresh や follow-up commit 後は、過去コメントの success が current head SHA と一致しているかを確認する
- latest head に workflow run が未検出なら、current head の CI evidence は不足として扱い、再実行・follow-up commit・maintainer への差し戻しのどれが必要かを分ける
- visual evidence blocker と CI evidence blocker を混ぜず、browser-capable evidence が必要な PR では CI green だけで完了扱いにしない
- combined status が空でも、GitHub Actions の workflow run / job 一覧を確認する
...
- `security-audit` failure を Ruby / root JavaScript / Docusaurus のどの dependency tree の問題かに分け、別 tree の PR へ混ぜていないか
- `security-audit` / `build-docs` が通常 human PR で skipped の場合、failure でも確認済み green でもなく、job 条件どおりに未実行だったと切り分けられているか
...
- `docs/specs/publish.json 仕様と生成ルール.md`
- `docs/guides/ローカル編集からポータル更新までの最小運用案.md`
