...
新しい確認ポリシーや通知仕様はここでは定義しません。確認依頼を作る入口、案件横断一覧、文書配下一覧、詳細で見える項目、`OK` / `Cancel` 後の戻り方を、current controller / view の挙動に合わせて整理します。dashboard 上の導線全体の役割差は [ダッシュボードと文書ショートカット・確認依頼の使い分け](../../specs/ダッシュボードと文書ショートカット・確認依頼の使い分け.md) を参照してください。

ここで扱う確認依頼は、文書に紐づく軽量な確認依頼の current flow です。画面上に `正式な確認依頼` と出る場合でも、正式レビュー承認 workflow の採否、承認者 chain、通知、SLA、段階承認、公開承認 policy を定義するものではありません。workflow 全体へ広げる必要が出た場合は [正式レビュー承認 workflow 境界メモ](../../specs/正式レビュー承認workflow境界メモ.md) に戻します。
...
一覧は default 50 件ずつ表示します。`表示 A-B件 / 条件一致 N件（Page X / Y、50件ずつ）` を見て、いまの status、free-text、依頼者、確認相手 filter に一致したうち何件目を見ているかを確認します。`次へ` / `前へ` は status、検索語、依頼者、確認相手を維持します。`per_page` を明示した場合も上限は 100 件で、`per_page=200` のような値は 100 件に丸められます。

status 未指定時の `対応待ち` / `処理済み` section は、現在 page に表示されている依頼だけを分けます。過去 page に pending があるか、処理済みがあるかを全体保証する表示ではありません。状態ごとの総数を見るときは状態ボタンの件数、現在 page の内訳を見るときは section 件数として読み分けます。
...
`依頼者` と `確認相手` は、現在の確認依頼に含まれる user だけを候補にする bounded search として読む。候補は名前 / email の検索語で最大 20 件まで返り、全体一覧では全体一覧の base relation、文書配下一覧ではその文書配下の base relation に含まれる requester / approver だけが対象になる。URL や detail の戻り先で指定済みの requester / approver は、通常候補の上限外でも selected endpoint から復元されるため、候補 dropdown に最初から出ない user が選択中 label として表示されることがあります。

この担当者 filter は、候補範囲と selected 復元のための UI 補助です。status、free-text、依頼者、確認相手は AND 条件で絞り込まれ、担当者候補に出ない user を新しく追加したり、確認相手を変更したり、通知や担当割り当て workflow を始める操作ではありません。
...
- 一覧の `依頼名` link から detail に入った場合、`一覧へ戻る` group の `一覧へ戻る` は元の一覧 URLへ戻る
...
- page 2 以降や明示 `per_page` で detail に入った場合も、`return_to` は現在の page / per_page と filter 条件を保持します。対応後に detail へ戻ったあと、`一覧へ戻る` で同じ page 文脈へ戻れます
...
- 依頼者 / 確認相手の候補に出ない user を見直したい: 全体一覧と文書配下一覧のどちらを見ているか、候補検索語が名前 / email に合っているかを確認する。URL で指定済みの user が base relation に含まれていれば、候補上限外でも選択中 label として復元される
...
- 条件一致件数が多い: `次へ` / `前へ` で 50 件ずつ確認する。急いで対象を探す場合は、検索語、依頼者、確認相手、status filter を先に絞る
- page 2 以降で detail へ入り、同じ場所へ戻りたい: 一覧の `依頼名` link から入る。detail の `一覧へ戻る` は page / filter 文脈を保持します
...
- 確認依頼を正式レビュー承認 workflow、公開承認、送付承認へ広げたい: この runbook では決めず、[正式レビュー承認 workflow 境界メモ](../../specs/正式レビュー承認workflow境界メモ.md) に戻して human decision として扱う
...
- `spec/requests/document_approval_request_pagination_spec.rb`
- `spec/requests/document_approval_request_section_count_cues_spec.rb`
- `spec/requests/document_approval_request_user_filter_search_spec.rb`
