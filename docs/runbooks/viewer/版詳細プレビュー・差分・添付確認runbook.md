...
`HTML本文を開く` から入る standalone viewer では、Markdown table の first slice として real HTML `<table>` ごとに stable key と wrapper metadata が付いています。これは table UX 拡張に使える seam ですが、full `rails_table_preferences` UI や preference schema の全面統合を意味しません。column visibility / preset UI / renderer 全体の変更は active queue に置かず、current fallback で具体的な不足が再現した場合に新しい concrete Issue として切り出します。
