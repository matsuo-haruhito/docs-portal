...
- 一覧は `accessed_at desc, id desc` の順で、1 ページあたり 50 件まで表示する
- 50 件を超える場合は `次の50件` / `前の50件` で page 移動でき、page link は既存 filter 条件を維持する
- `page` は最大 200 ページまでを採用して従来どおり最大 10,000 行へ到達できる。`limit` param では表示件数や取得上限を広げない
...
- `表示中ページをCSV export（最大50件）` は、`csv_scope=current_page` と現在の `page` を明示した場合だけ、同じ filter 条件の表示中 page 最大 50 件を固定列で出力する
...
- filter formでは `操作` `対象種別` `案件` を常時表示し、`company_id` `user_id` `q` `document_q` `from` `to` `ai_context_mode` `ai_context_scope` は同じGET form内の `高度条件` Disclosureへ格納する。高度条件が有効な場合だけ初期表示を開く
- `現在の条件でCSV export（最新200件）` は常時表示し、表示中ページCSV、2種のmetadata JSON、scope説明は初期状態を閉じた `export補助` Disclosureから確認する
...
- `from` / `to` は `accessed_at` の日付範囲で絞り込み、指定後も一覧の並び順と 1 ページ 50 件表示は変わらない
...
- 3 つを組み合わせると、対象をかなり狭くして 50 件単位のページ内で追いやすくなる
...
- 期間指定後も、表示は `accessed_at desc, id desc` の新しい順で 1 ページ 50 件までに留まる。200 件を超える場合は、次ページへ進むか、期間を短くするか、他の条件を足して絞り込む
...
- filter ありで 0 件の場合は `条件に一致する監査ログはありません。` と表示される。画面上部の filter form だけでなく、empty state 近くにも `条件をクリア` が出るため、まず有効な条件 badge を見直し、条件を外して最新 50 件へ戻れる
...
一覧は 50 件固定の page として読む。条件に一致する証跡が 50 件を超える場合、画面上部に `次の50件` が出る。

- `次の50件`: 現在の filter 条件を保ったまま、より古い 50 件へ進む
- `前の50件`: 現在の filter 条件を保ったまま、新しい側の 50 件へ戻る
- `page` は内部的なページ番号で、最大200ページまでに制限される。1ページ50件へ縮小しても最大10,000行の到達範囲は維持する
- `limit` を渡して表示件数や到達範囲を増やすことはできない
...
- `表示中ページをCSV export（最大50件）`: `csv_scope=current_page` を明示したときだけ、現在の filter 条件と `page` に一致する表示中 page 最大 50 件を出力する。2 ページ目で押すと、同じ条件の 2 ページ目だけを出力する

古い page の証跡をそのまま共有・保管したい場合は、`次の50件` などで対象 page を表示してから `表示中ページをCSV export（最大50件）` を使います。日次や案件単位の最新証跡を共有したい場合は、従来どおり `現在の条件でCSV export（最新200件）` を使います。
...
`page` が不正、または上限を超える場合の表示中 page CSV は、一覧の page 解釈と同じく first page 相当へ丸められ、`limit` param で 50 件上限は広がりません。
...
- 表示中 page の 50 件を共有・保管したいとき、`現在の条件でCSV export（最新200件）` ではなく `表示中ページをCSV export（最大50件）` を選べているか
...
- 目的の証跡が見つからないとき、1 ページ 50 件の範囲からあふれていないか。必要なら次ページへ進むか、条件を足して絞り込む

## 変更時の注意

- 期間指定と page 移動は一覧の絞り込み・表示補助であり、監査ログ保存期間や retention policy は変えない
- current UI は 50 件単位の page 移動、現在の filter 条件に一致する最新 200 件の CSV export、表示中 page 最大 50 件の CSV export を持つ。最大200ページ・10,000行の到達範囲は維持するが、無制限全件取得、任意の `limit` 指定、scheduled report は current support として扱わない
...
- 条件ありで 0 件になった: empty state 近くの `有効な条件` を見直し、条件を少し緩めるか、`条件をクリア` で query なしの最新 50 件へ戻る
- 一覧が広くて読みづらい: 表示設定で必要な列だけを残す
- 条件に一致する最新 200 件を持ち出して確認したい: `現在の条件でCSV export（最新200件）` を使い、固定列と表示設定の違いを確認する
- 表示中 page の最大 50 件を持ち出して確認したい: 対象 page を開いたうえで `表示中ページをCSV export（最大50件）` を使う
- CSV の条件や scope だけ先に確認したい: 最新 200 件なら `CSV条件metadata JSON`、表示中 page なら `表示中ページmetadata JSON` を見る
- 50 件より古い証跡を追いたい: まず filter を維持したまま `次の50件` へ進み、広すぎる場合は期間や対象条件を足す。古い page の証跡を出したい場合は、その page を開いてから表示中ページ CSV を使う
