...
- 表示できるユーザーが 0 件のときは、検索結果 summary に `表示できるユーザーがないため、列の表示設定は表示していません` と出る。これは列設定の不具合や権限不足ではなく、対象行がないため `ユーザー一覧の表示設定` を出していない cue として読む
- 一覧は bounded pagination で表示され、通常は 1 ページ 25 件ずつ確認する。URL の `per_page` は最大 100 件までに丸められる
- 一覧の上に出る `表示中: X-Y件 / N件` は、現在ページで表示している範囲と filter 後総件数を分けて読む
- 複数ページある場合は `前へ` / `次へ` と `現在ページ / 総ページ` が出る。page 移動時も `q` / `active` / `per_page` は維持される
...
- 登録済みユーザーはいるが filter 条件に一致しないときは、`検索条件に一致するユーザーはありません。` と表示され、条件変更または `条件をクリア` を促す。filter 適用中は empty state の中にも `条件をクリア` link が出る。`条件をクリア` はキーワード / 状態 filter の reset であり、ユーザー削除、権限変更、表示設定保存の取り消しではない
...
- `適用中` と `検索結果` を見て、現在の絞り込み条件と filter 後総件数を確認する
- 表示できるユーザーが 0 件のときは、検索結果 summary と empty state を合わせて読み、`ユーザー一覧の表示設定` が出ないのは対象行がないためだと確認する
- `条件をクリア` は現在のキーワード / 状態 filter を外してユーザー一覧へ戻る導線として使う。ユーザー登録、削除、権限、表示設定保存を変更する操作ではない
- `表示中` と `前へ` / `次へ` を見て、現在ページの範囲と次に確認するページを把握する
...
- `検索条件に一致するユーザーはありません。` が出る: 自社 scope 内に登録済みユーザーはあるが、現在の keyword / 状態 filter に一致していない。条件変更か `条件をクリア` を使う。この状態では `ユーザー一覧の表示設定` は表示されず、検索結果 summary の `表示できるユーザーがないため、列の表示設定は表示していません` を対象行 0 件時の説明として読む
- ユーザー画面の `条件をクリア` を使う: キーワード / 状態 filter を外して一覧へ戻るための reset であり、ユーザー削除、権限変更、表示設定保存の取り消しではない
