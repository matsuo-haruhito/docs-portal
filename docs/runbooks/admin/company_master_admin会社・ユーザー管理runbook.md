...
- 表示できる会社が 0 件のときは、検索結果 summary に `表示できる会社がないため、列の表示設定は表示していません` と出る。これは列設定の不具合ではなく、対象行がないため `会社一覧の表示設定` を出していない cue として読む
...
- 一覧には自社ユーザーだけが出る
- 画面見出し横の `新規登録` は初期状態を閉じた Disclosure で、validation error 後だけ入力内容とエラーを見せた状態で開く
- filter toolbar のキーワードと状態 filter を使って表示中 scope のユーザーを絞り込める
- キーワードは `name` と `email_address` の断片に一致するユーザーを探す。入力欄の placeholder はユーザー名・メールアドレスで、最大100文字。表示名は一覧列として確認する
- 状態 filter は `すべて` / `有効` / `無効` を選べる
- 検索や状態 filter を使っても、`company_master_admin` は自社ユーザー scope を越えない
- filter 適用中は toolbar の下に filter chip が出る。キーワードと状態を両方使う場合は、それぞれ独立した chip として現在値を確認できる
- 件数表示の `N件` は現在のキーワード・状態 filter に一致するユーザーの総件数を表す。列の表示設定を変えても、この件数や検索条件、権限 scope は変わらない
- filter toolbar の `クリア` は現在の条件をすぐ外す導線。0件の empty state にある `条件をクリア` も同じ条件なし一覧へ戻る導線で、ユーザー削除、権限変更、表示設定保存の取り消しではない
- 一覧は bounded pagination で表示され、通常は 1 ページ 25 件ずつ確認する。URL の `per_page` は最大 100 件までに丸められる
- 複数ページある場合は `前へ` / `次へ` と `現在ページ / 総ページ` が出る。page 移動時も `q` / `active` / `per_page` は維持される
- 一覧の `編集` / `削除` link は、現在の検索条件・page を `return_to` として持つ。更新 / 削除後は安全な内部 path だけへ戻り、外部 URL や unsafe path はユーザー一覧へ fallback する
- 一覧には `ユーザー一覧の表示設定` があり、列の表示状態を調整できる。表示設定は一覧列だけを変えるもので、検索条件、pagination、ユーザーの登録・削除、table preferences の保存 contract は変えない
- filter 後のユーザーが 0 件のときは空 table と列設定を描画せず、empty state を表示する
- ユーザー自体が 0 件のときは、見出し横の `新規登録` Disclosure を開いてメールアドレスと必要な項目を入力する
...
- filter chip で現在のキーワード / 状態を、`N件` で filter 後総件数を確認する
- filter 後 0 件では empty state を読み、`ユーザー一覧の表示設定` と table が出ないのは対象行がないためだと確認する
- toolbar の `クリア` または empty state の `条件をクリア` は、現在のキーワード / 状態 filter を外してユーザー一覧へ戻る導線として使う。ユーザー登録、削除、権限、表示設定保存を変更する操作ではない
- `前へ` / `次へ` と `現在ページ / 総ページ` を見て、同じfilterの次に確認するページを把握する
...
- `検索条件に一致する会社はありません。` が出る: 自社会社 scope 内に登録済み会社はあるが、現在の keyword / 状態 filter に一致していない。条件変更か `条件をクリア` を使う。この状態では `会社一覧の表示設定` は表示されず、検索結果 summary の `表示できる会社がないため、列の表示設定は表示していません` を 0 件時の説明として読む
...
- 自社ユーザーを探したい: ユーザー一覧の filter toolbar で名前・メールアドレスの断片や有効/無効状態を絞り込む。キーワード入力欄は最大100文字で、表示名は検索対象ではなく一覧列として確認する
- ユーザー画面に filter chip が出ている: keyword または状態 filter がかかっている。隣の `N件` は現在の filter 後総件数であり、列の表示設定では変わらない
- 複数ページある: `現在ページ / 総ページ` と `前へ` / `次へ` を使い、同じ条件のまま移動する
- ユーザー一覧から編集・無効化する: filter や page を使っている場合、`編集` / `削除` は現在の一覧条件を戻り先として持つ。保存後に戻り先が外部 URL になることはない
- ユーザー一覧が 0 件になる: 自社 scope 内に登録済みユーザーがある場合は条件不一致の empty state、ユーザー自体がない場合は未登録の empty stateとして読む。条件不一致では toolbar の `クリア` または empty state の `条件をクリア` を使う
- ユーザー画面の `クリア` / `条件をクリア` を使う: キーワード / 状態 filter を外して一覧へ戻るための reset であり、ユーザー削除、権限変更、表示設定保存の取り消しではない
