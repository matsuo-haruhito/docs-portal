...
- `previewで使用中`: current preview がこの接続を使います。通常は `enabled` かつ `preview_selected` の明示選択です。
- `有効だが未使用`: この行も有効ですが、別の接続が明示選択されているか、旧データの互換 fallback で別行が使われています。legacy duplicate を整理するときの対象です。
- `previewでは未使用`: 無効な接続、または preview の対象外の接続です。

current `main` では、フォームの `previewで使用する` で案件ごとの利用接続を明示します。`preview_selected` は案件ごとに最大 1 件で、無効な接続は選択できません。選択中の接続を無効化すると選択状態も解除されます。

明示選択がない旧データだけは、同一案件の有効接続のうち最小 DB id を互換 fallback として `previewで使用中` にします。この fallback は旧行を壊さず読み込むための境界であり、新規運用の選択方法ではありません。通常は対象行を編集して `previewで使用する` を選び、一覧で結果を確認します。

## 複数行が並んでいるときの見方

- 一覧は `name`, `id` 順で並びます。
- 新規保存では同一案件の有効接続を 1 件に制限し、`preview_selected` も案件ごとに 1 件だけです。
- 旧データ由来で同一案件に有効行が複数残っている場合は、一覧上部の整理 card と `preview利用` 列を見て、実際に使う接続を決めます。
- legacy duplicate を整理するときは、残さない接続を先に無効化し、残す接続の `Drive ID`、`プレビュー用フォルダ`、`Tenant / Client` を確認して `previewで使用する` を選びます。
- 明示選択前は最小 DB id の互換 fallback が使われる場合がありますが、DB id が小さい行を正しい接続とみなして運用を確定しません。
- 日常運用では `previewで使用中` を 1 件だけ維持し、`有効だが未使用` を放置しない状態に戻します。
...
- `状態`: `有効` / `無効` の切り替えが案件運用と一致しているか。無効化すると preview の明示選択も解除される
- `previewで使用する`: この案件の Office preview に使う有効接続か。案件ごとに 1 件だけ選ぶ

フォーム下部では、同一案件で有効にできる Microsoft Graph 接続と `previewで使用する` 接続がそれぞれ 1 件だけであることを確認します。切り替える場合は不要な有効接続を先に無効化し、残す接続を有効かつ preview 利用対象として保存してください。

## preview 不達時の戻り先

1. まずこの一覧で、対象案件に `previewで使用中` の行があるか確認する
2. `preview 利用中の接続を要整理の案件` card や `有効だが未使用` 行が出ていないか確認し、明示選択があるか、旧データの最小 DB id fallback に留まっていないかを見る
