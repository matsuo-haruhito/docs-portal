...
- [Microsoft Graph接続管理 runbook](./Microsoft%20Graph%E6%8E%A5%E7%B6%9A%E7%AE%A1%E7%90%86runbook.md): 案件ごとの enabled 状態、Drive ID、プレビュー用フォルダ、preview 不達時の戻り先
...
- [外部フォルダ同期dry-run・apply運用 runbook](./外部フォルダ同期dry-run%E3%83%BBapply%E9%81%8B%E7%94%A8runbook.md): `最新安全判定` / `競合・重複警告` / `dry_run` / `apply` / `force_apply` の見分け方
