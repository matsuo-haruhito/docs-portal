...
- [外部フォルダ同期dry-run・apply運用runbook](./外部フォルダ同期dry-run%E3%83%BBapply%E9%81%8B用runbook.md): provider-aware な入口、`最新安全判定` / `競合・重複警告`、Google Drive の current support、SharePoint / OneDrive の metadata 保存 first slice と未対応の同期本体
