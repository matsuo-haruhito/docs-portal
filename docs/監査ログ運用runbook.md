...
- 目的の証跡が古い場合でも、まず案件・会社・ユーザー・文書名・期間で絞り、必要に応じて page 移動する

AI context mode / scope、日付、案件、会社、ユーザー、文書名の filter は page link に引き継がれる。page 移動後に条件が外れていないかは、`有効な条件` の badge と表示件数で確認する。
