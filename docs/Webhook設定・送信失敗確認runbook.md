...
- 送信履歴の `詳細` と行ごとの `再送` は、現在の status filter を安全な戻り先として引き継ぎます。`失敗` 表示から詳細へ入った場合、`Webhook一覧へ戻る` と 1 件再送後の戻り先は `失敗` 表示です。
- 不正な戻り先 filter や任意 URL は引き継がず、通常の Webhook 一覧へ戻します。
- `失敗` 表示中のまとめて再送は、表示中の最大 50 件のうち failed かつ endpoint が有効な delivery だけを対象にします。
- 50 件外の履歴まで探す必要がある場合は、`admin/webhook_deliveries` の `Webhook送信履歴検索` を開き、endpoint / event type / status / 作成日で条件を指定します。この検索一覧は read-oriented な調査導線であり、まとめて再送の対象は広げません。
...
送信履歴の表示設定を絞る場合も、状態確認では `ステータス` と `操作`、失敗調査では `HTTP` と `エラー` を残してください。`詳細` へ入ったあとの戻り先は status filter だけを保持するため、表示設定で列を隠しても delivery の詳細情報や再送条件そのものは変わりません。

`Webhook送信履歴検索` では、50 件外や特定条件の履歴を endpoint / event type / status / 作成日 range で探せます。検索結果は新しい順の上限付き表示です。検索結果から `詳細` へ入った場合、一覧の query を安全な戻り先として保持します。不正な `return_to` や外部 URL は採用しません。
...
2. `送信履歴` を `失敗` に絞り込み、表示範囲の総件数と表示件数を確認します。50 件外まで調べる必要がある場合は、`Webhook送信履歴検索` で endpoint / event type / 作成日を指定して調査してください。
3. 失敗行の `詳細` を開きます。`Webhook一覧へ戻る` または `Webhook送信履歴へ戻る` は元の一覧へ戻るため、複数件を順に確認する場合も同じ条件から再開できます。
...
6. 1 件再送は、詳細画面から実行しても元の status filter または検索一覧へ戻ります。`失敗` 表示から詳細へ入った場合は、再送後も `失敗` 表示で結果確認を再開できます。
