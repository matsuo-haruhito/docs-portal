...
- 不正な戻り先 filter や任意 URL は引き継がず、通常の Webhook 一覧へ戻します。検索条件や paging の保持は current scope ではありません。
...
3. 失敗行の `詳細` を開きます。`Webhook一覧へ戻る` は元の `失敗` filter へ戻るため、複数件を順に確認する場合も同じ失敗一覧から再開できます。
...
6. 1 件再送は、詳細画面から実行しても元の status filter 一覧へ戻ります。`失敗` 表示から詳細へ入った場合は、再送後も `失敗` 表示で結果確認を再開できます。
