...
2. `送信履歴` を `失敗` に絞り込み、表示範囲の総件数と表示件数を確認します。50 件外まで調べる必要がある場合は、`送信履歴検索へ` から endpoint / event type / status / HTTP status / error message 断片 / 作成日で探します。作成日 From / To の warning が出ている場合は、その日付条件だけが外れているため、他の filter が意図どおりかも合わせて確認します。
