...
案件が増えてきたら、`案件を探す` でキーワード、状態、企業を使って一覧を絞り込む。条件を入れて 0 件になった場合は、案件が削除されたと決めつけず、結果一覧側の empty state 近くに出る `条件をクリア` から条件なし一覧へ戻る。検索フォーム内の `条件をクリア` と同じく、filter 条件を外すための導線であり、Project の保存内容、企業ひも付け、削除状態、表示設定を変更するものではない。
