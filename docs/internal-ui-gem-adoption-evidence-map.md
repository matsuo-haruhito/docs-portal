...
| --- | --- | --- | --- | --- |
| `tree_view` | `app/views/documents/_tree.html.erb`、`app/views/projects/_document_detail_tree.html.erb`、`spec/requests/document_tree_regressions_spec.rb` の sidebar tree / detail tree / persisted state / window offset | `tree_view-rails` の `README.md`、`docs/ja/README.md`、`docs/ja/installation.md`、`docs/ja/usage.md`、`docs/ja/api.md`、`docs/ja/public-api.md`、`docs/ja/release.md`、`config/public_api_manifest.yml`、`docs/mockups/review-gallery.html`。downstream host app evidence 節では、TreeView 側の release evidence と docs-portal 側 smoke / rollback note を分けて読む | 1. pinned ref と upstream public API / manifest / release evidence / mockup を確認する。2. `DocumentsHelper` / `ProjectsHelper` / tree partial の render state を確認する。3. sidebar tree と detail tree の両方を smoke する。片方だけなら未確認側を明記する。4. route、permission、icon、business row action は host app 側責務として rollback note に分ける。 | from / to SHA、sidebar tree と detail tree のどちらを見たか、persisted state / selection / window offset の確認結果、戻す ref、query / permission / icon / route / business row action は docs-portal 側責務であること |
...
- `tree_view` の release evidence は upstream repository の public API manifest、release docs、package verification、mockup / browser smoke を正本にし、docs-portal の sidebar/detail tree smoke は host app adoption evidence として分けます。
