...
0 件になったときは、filter なしなら manual_upload dry-run がまだ作られていない状態です。filter ありなら、控えている `dry_run_id`、案件、状態のいずれかが current dry-run と合っているかを見直し、必要なら `絞り込み解除` で一覧に戻ります。source name / path / hash の全文検索、ZIP dry-run との横断検索、artifact import dry-run との統合一覧は current support ではありません。
...
- admin UI で manual upload dry-run を確認する場合、この runbook は `index` / `show` / `update` の current behavior と API response の照合順を扱います。source name / path / hash の検索拡張、0 件表示の改善、raw `source_path` の表示範囲は別 issue の判断に戻します
