...
| `file_uploads` | 同期クライアントや単体ファイルの手動取り込み | `file` を付けて送る。`validate_only=true` は省略できる | `import_dry_run_id` を付けて同じ `file_uploads` へ送る。管理画面では `admin/file_upload_dry_runs/:public_id` で確認して実行できる |
...
- `案件`、`状態`、`dry-run ID`
- `source_name`、`relative_path`、`source_path`
...
API response と画面を照合するときは、まず `dry_run_id` と画面の `dry-run ID` が一致することを確認します。次に `relative_path`、`content_hash`、`source_commit_hash`、warning / error、TreeView の差分を見ます。`source_path` は取り込み先を決める値ではなく、同期元やクライアント側の参考情報です。private path や端末固有の path を大きく露出しない方針が必要になった場合は、表示範囲の判断を #1613 に戻します。
...
- 管理画面で dry-run が開けない: `admin/file_upload_dry_runs` は `import_mode: manual_upload` の dry-run だけを見る。ZIP dry-run は [ZIPインポートdry-run運用 runbook](./ZIP%E3%82%A4%E3%83%B3%E3%83%9D%E3%83%BC%E3%83%88dry-run%E9%81%8B%E7%94%A8runbook.md) 側で確認する
...
`zip_uploads` で作った dry-run は `zip_uploads` へ、`file_uploads` で作った dry-run は `file_uploads` または `admin/file_upload_dry_runs` へ戻します。
...
- admin UI で manual upload dry-run を確認する場合、この runbook は `show` / `update` の current behavior と API response の照合順だけを扱います。後から探せる一覧入口は #1614、raw `source_path` の表示範囲は #1613 の判断に戻します
