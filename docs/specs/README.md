...
このディレクトリの仕様文書は、段階的に `.kiro/specs/{feature}/` へ移行中です。

- **新規仕様** は `.kiro/specs/{feature}/`（requirements.md / design.md / tasks.md 形式）に作成する
- **既存文書** は次に実装で触るタイミングで `.kiro/specs/` へ移行する（一括移行は行わない）
- 移行後はこの README のリンクを `.kiro/specs/` 側へ差し替える
