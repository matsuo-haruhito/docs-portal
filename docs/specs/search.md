...
文書一覧のキーワード入力欄では、検索対象が file name だけでなく source path / attachment path にも広がっていることを示すため、「文書名・URL識別子・元パス・版・本文・キーワード・添付ファイル名/パス」と案内する。

TreeView のフォルダから文書一覧へ遷移する場合は、フォルダ path が `q` に入ることがある。`q` が `docs/specs` のような path 形式の場合、検索条件として扱いつつ、アップロードUI上でも「そのフォルダ直下への追加候補」と分かる表現を優先する。

一覧の「ヒット理由」列では、match label に加えて先頭 3 件までの短い一致値を表示する。長い値は 80 文字程度に丸め、検索結果の理由を推測しやすくする。

0件の場合は文書テーブルや ZIP 出力フォームを表示せず、空状態として次を案内する。

- 検索語がある場合は、その検索語に一致する閲覧可能文書がないこと
- 検索語を短くする、別名・ファイル名・タグで探すこと
- タグ、カテゴリ、ファイル種、公開範囲、実用フィルタを外すこと
- 外部ユーザーの場合は担当者に権限確認を依頼すること
- 内部ユーザーの場合はアップロード導線を残すこと
