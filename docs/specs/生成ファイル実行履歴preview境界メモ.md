...
このメモは、`生成ファイル実行履歴` index の検索欄と detail の下部に出る診断ブロックを読むときの補助です。基本の切り分け順、filter、retry の操作境界は [生成ファイル再試行と定期ジョブ管理 runbook](../runbooks/ops/生成ファイル再試行と定期ジョブ管理runbook.md) を正本にします。
...
詳細画面の `一覧 q 検索の手掛かり` は、detail で見えている run から一覧検索へ戻るための短い候補カードです。実行ID、ジョブID、関連イベントID、入力パス / 変更ファイル / 生成パスの basename、エラー先頭行のうち、安全に q 検索へ渡せる候補だけを表示します。
