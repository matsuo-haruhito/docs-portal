...
- 運用確認順: [build-docs workflow確認runbook](../runbooks/import/build-docs workflow確認runbook.md)
...
## current workflow evidence の読み方

後続 issue を切る前に、current `build-docs` job の input / output / artifact / workflow run evidence を次の粒度で読む。

- input: `docusaurus/package-lock.json`、`docusaurus/` source、`publish/documents.json`、`attachments/`、workflow context の `github.repository` / `github.ref_name` / `github.sha` / `github.run_id` / `github.run_attempt`
- output: `docusaurus/build`、`publish/manifest/publish.json`、`docs-site.tar.gz`
- artifact evidence: artifact 名 `docs-site`、archive に含まれる `docusaurus/build` / `attachments` / `publish/manifest`
- workflow run evidence: workflow 名 `ci`、job 名 `build-docs`、run id、run attempt、source branch / commit、job conclusion、import API step が条件未達か実行済みか
- import handoff evidence: `RAILS_PORTAL_IMPORT_URL` と `RAILS_PORTAL_IMPORT_TOKEN` がある `main` push のときだけ `artifact_root` と `manifest_path` を app に渡す

この evidence は、Rails app 側 job 化の採否を決めるものではありません。後続 issue で「どの事実を置き換えるのか」を誤読しないための inventory として扱います。
...
後続 issue は、次のように 1 issue = 1 採否判断または 1 実装 surface に分けます。

| 後続 issue の切り方 | 含める判断 | 含めない判断 |
| --- | --- | --- |
| Docusaurus build 実行場所 | GitHub Actions に残すか、Rails app 側 job 化するか | artifact retention、replay UI、import API pull 型化 |
| Artifact / manifest metadata 保存 | run id、attempt、commit、manifest path、document count などの short metadata | artifact 本体、manifest 全文、attachments 本文、CI log 全文 |
| Import handoff 方式 | current push 型 API call を残すか、app 側 pull / fetch 型へ分けるか | queue / worker / retry / alert の詳細実装 |
| Replay / rebuild 運用 | 同一 artifact replay と source rebuild の判断入口 | 自動 retry、scheduled job、通知先、SLA |
| Retention / access control | GitHub Actions artifact 以外へ保存するか、誰が見られるか | Docusaurus build 実行場所や import schema 変更 |
