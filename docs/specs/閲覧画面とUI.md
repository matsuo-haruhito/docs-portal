...
- `admin` は管理画面の `admin/document_sets` 一覧で `rails_table_preferences` の表示設定 UI を開ける
- `admin/document_sets` 一覧の表示設定は `admin_document_sets` を table key とし、案件、名称、種別、公開範囲、文書数、操作列を対象にする
