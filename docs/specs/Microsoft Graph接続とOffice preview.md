...
| client_secret | 必須 | アプリケーション認証に使います。保存時は暗号化カラムです。編集時の共有 URL 解決では、空欄なら保存済み secret を使います。 |
...
SharePoint / OneDrive の共有フォルダ URL が分かる場合は、`共有フォルダURL` に入力して `共有URLからDrive情報を解決` を押すと、保存前に `drive_id`、`site_id`、`preview_folder_path` の候補をフォームへ戻せます。このボタンは保存ではありません。候補値を確認し、接続設定として反映したい場合は別途 `保存` を押します。

解決時も `tenant_id`、`client_id`、`client_secret` は必要です。編集画面では、secret を変更しない限り `Client secret` を空欄のままにしても保存済み secret で解決できます。新規登録では secret を入力してから解決してください。

現行の入力補助は共有フォルダ URL を前提にしています。ファイル共有 URL、無効な URL、Graph 側で drive item を取得できない URL はエラーとしてフォームに戻ります。この機能は Office preview 用の一時アップロード先を埋めるための補助であり、SharePoint / OneDrive との双方向同期や delta 取得の仕様ではありません。
...
1. 管理画面で対象案件の `Microsoft Graph接続` を登録する
2. 新しく有効化する前に、同一案件で別の `enabled` 接続が残っていないかを確認する
3. 共有フォルダ URL から候補を取りたい場合は、保存前に `共有URLからDrive情報を解決` で `Drive ID` と `プレビュー用フォルダ` を確認する
4. 対象案件に Office ファイルを持つ文書版を用意する
5. 一覧の `preview利用` 列で `previewで使用中` の行を確認する
6. 文書詳細や版詳細から Office ファイルの inline preview を開く
7. Graph preview が開くことを確認する
8. 必要なら、Graph 接続を無効化した状態または Graph 側失敗時に Google Drive fallback が成立するか確認する
...
- 同一案件で別の `enabled` 接続が既に残っている

### 共有URLからDrive情報を解決できない

- `共有フォルダURL` が空欄、HTTP URL として解釈できない値、またはファイル共有 URL になっている
- `tenant_id`、`client_id`、`client_secret` が不足している
- Microsoft Graph の access token 取得、または `shares/.../driveItem` 取得に失敗している
- 解決できた drive item から root 相対の preview folder path を組み立てられない

### preview が開かない

- 対象案件に `previewで使用中` の Microsoft Graph 接続がない
- 一覧上部の整理 card や `有効だが未使用` が残っており、どの接続が使われるかを見誤っている
