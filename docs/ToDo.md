...
- ここに書くもの: 将来の検討事項、人間判断待ち、未起票の要望、具体 Issue への導線
- ここに書かないもの: gem 展開・Stimulus 化など技術基盤の短期アクション → [ROADMAP.md](../ROADMAP.md)
...
- 正式なレビュー・承認ワークフロー導入は、コメント・品質チェック・公開制御・送付運用が固まってから再評価する。分類: 人間判断待ち。現在の確認依頼、コメント、品質チェック、公開制御、外部送付履歴を、多段承認、通知、SLA、権限変更、公開承認 state machine の実装済み workflow として扱わない。
