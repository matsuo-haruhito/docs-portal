...
初回状態では、上段 `新規登録` フォームの下に `同意文面一覧の表示設定` と table header だけが続くことがある。まだ empty state は出ないので、最初の 1 件は上段フォームから作る。

1. `タイトル` と `版` を入れる
2. `種別` と `再同意方針` を current 運用に合わせて選ぶ
3. 利用者へ見せる本文を `body` に入れる
4. すぐ利用する文面なら `有効` を維持したまま保存し、下段一覧に `タイトル / 版 / 種別 / 状態` が出たことを確認する

補足:

- 改訂時は既存行を上書きせず、新しい `version_label` の行を追加して版を分ける
- 利用履歴や案件設定が残る文面は削除できないため、廃止したいときは一覧で `無効` 状態へ切り替える運用を前提にする
...
初回状態では、上段 `新規登録` フォームの下に `案件同意設定一覧の表示設定` と table header だけが続くことがある。まだ empty state は出ないので、最初の 1 件は上段フォームから作る。

1. `案件` を選ぶ
2. 先に `有効` 化してある `同意文面` を選ぶ
3. `必須タイミング` は current enforcement 済みの `first_access` か `download` を優先して選ぶ
4. 保存したら、下段一覧に `案件 / 同意文面 / 版 / 必須タイミング / 状態` の行が追加されたかを確認する

補足:

- `同意文面` 候補に何も出ないときは、先に `同意文面` 画面で active な文面を 1 件作る
- `shared_link_access` と `shared_link_download` は予約値なので、現行運用の enforcement を見たいときは `first_access` と `download` を基準に読む
