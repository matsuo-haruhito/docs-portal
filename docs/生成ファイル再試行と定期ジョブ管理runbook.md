...
current main の定期ジョブ一覧には、`job_key` / `job_class` / エラー断片を対象にした検索欄はまだありません。手元の識別子から探す必要がある場合は、一覧の表示列と前回状態 filter で候補を絞り、詳細画面の情報で確認します。
...
`実行履歴` は最新 50 件を表示します。古い run まで含めた全件探索画面ではないため、過去の失敗を追うときは、まず前回状態と最新履歴で現在の症状を切り分けます。履歴の `状態` filter は詳細画面内の run 表示にだけ効き、一覧側の前回状態 filter とは役割が異なります。
