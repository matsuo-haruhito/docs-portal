...
方針の正本は [フロントエンド操作の方針](../.kiro/steering/frontend-interaction-policy.md)、調査時に読むファイルと upstream docs の入口は [関連 gem 連携調査 runbook](./関連gem連携調査runbook.md) です。この文書では、代表画面、gem 側責務、host app 側責務、確認すべき contract を 1 表に寄せます。
