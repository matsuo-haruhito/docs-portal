...
- `出力モード`: HTML preview の主表示は `概要中心` / `本文込み`。補助表示として `mode: compact` または `mode: full` を併記する
...
- selected scope のときは、同じ `出力` card に `選択 scope handoff` が表示される。`mode`、`scope: selected`、`選択ID`、`案件内候補`、`出力対象`、`候補外` の件数を、JSON / Markdown を押す前の read-only summary として読む
- `選択 scope handoff` の文書 table は、viewer が閲覧できる選択文書だけを `タイトル`、`slug`、`public_id` で表示する。案件外 ID、存在しない ID、閲覧不可文書の raw ID や public_id は表示しない
...
`target_name` の `mode` は、HTML preview 上の `概要中心` が `compact`、`本文込み` が `full` として保存されます。`scope` は `document_ids[]` がない場合に `all`、明示選択がある場合に `selected` です。`selected_count` は request で指定された文書 ID 数、`scoped_count` はそのうち現在の案件内文書として解決できた数、`exported_count` は viewer が実際に export できる文書数です。案件外または存在しない ID は候補外として数だけ扱い、文書 ID や title は AccessLog に残しません。必要な場合は preview の `明示選択` / `案件内候補` / `出力対象` summary と、`含まれる文書` / `除外された文書` / `選択 scope handoff` を照合します。

監査ログ画面では、`target_type=ai_context` を指定したうえで `AI context mode` と `AI context scope` を併用できます。mode は `compact` / `full`、scope は `全件` / `選択` で、どちらも `target_name` に保存された `mode=<value>;` / `scope=<value>;` を見る補助 filter です。`target_type` が `ai_context` でない場合、AI context 固有 filter は条件から外れます。
...
- selected scope では、`出力` card の `選択 scope handoff` で mode / scope / 選択ID / 案件内候補 / 出力対象 / 候補外の件数と、閲覧可能な選択文書の title / slug / public_id を確認できる
- `選択 scope handoff` は export 前の read-only summary であり、保存済み preset、権限変更、外部 AI 送信、文書本文の再表示ではない
...
検索は export 対象をその場で確定する操作ではありません。検索しただけでは、JSON / Markdown export link は現在の対象範囲を維持します。検索結果から対象を変える場合は、checkbox を確認して `選択した文書でpreview` を押し、HTML preview の `出力対象` 件数と `含まれる文書` / `除外された文書` / `選択 scope handoff` を見てから JSON / Markdown を出力します。
...
選択済みだけ表示中に、権限変更や公開状態変更で確認できる閲覧可能な選択文書が 0 件になった場合は、empty state に `検索候補へ戻る` と `すべての文書に戻す` が出ます。`検索候補へ戻る` は選択 scope を保ったまま通常候補へ戻って確認する導線、`すべての文書に戻す` は選択 scope 自体を解除する導線として読み分けます。候補 0 件は「export 全体が空」とは限らないため、`出力対象` 件数、`含まれる文書`、`除外された文書`、`選択 scope handoff` を合わせて確認します。
...
- HTML preview の `出力` card では、JSON / Markdown action 近くの `現在の対象` cue で all / selected scope と出力対象件数を押下前に読み返せます
- HTML preview の `選択 scope handoff` は、selected scope の mode / scope / count と閲覧可能な選択文書だけを title / slug / public_id で確認する read-only summary として扱います
...
- 監査ログの AI context mode / scope filter は `target_type=ai_context` の `target_name` を使う補助 filter であり、AccessLog schema 正規化や任意の `target_name` 全文検索ではありません
