...
| `rails_table_preferences` | `README.md` と `docs/index.md` family が helper、mounted JSON API、JavaScript entrypoints、manual QA、release checks、package verification を説明する | quick start、production integration checklist、install paths、support matrix、decision guide、troubleshooting、manual QA / release check docs を入口にする。release narrative drift guard がある場合は current merged docs だけを見る | `package.json` の `exports` は package root と `./controller` を持つ。package verifier / TypeScript declaration / release checklist の完了状況を upstream 側で確認する | `admin/document_sets` の editor、stable column key、filter / preset、mounted engine save、export / table preference 境界を代表 smoke として見る | table key、列 metadata、検索 filter、export semantics、Markdown preview table fallback は docs-portal 側責務。known-good revision 判断や preview table 採用判断は `#789` / 関連 issue に戻す |
