...
- キーワードは `domain` と `name` の断片に一致する会社を探す。画面上はドメイン・会社名・表示名を探す入口として扱う
...
- `検索結果: N件` は現在のキーワード・状態 filter 後に一覧へ出る会社数を表す。列の表示設定を変えても、この件数や検索条件、権限 scope は変わらない
...
- 対象会社をドメイン・会社名の断片や有効/無効状態で探す
- `適用中` と `検索結果` を見て、現在の絞り込み条件と一覧件数を確認する
...
- キーワードは `name` と `email_address` の断片に一致するユーザーを探す。画面上はユーザー名・表示名・メールアドレスを探す入口として扱う
- 状態 filter は `すべて` / `有効` / `無効` を選べる
- 検索や状態 filter を使っても、`company_master_admin` は自社ユーザー scope を越えない
- filter 適用中は form の下に `適用中: ...` と `検索結果: N件` が出る。キーワードと状態を両方使う場合は `キーワード「...」 / 状態: ...` のように並ぶ
- `検索結果: N件` は現在のキーワード・状態 filter 後に一覧へ出るユーザー数を表す。列の表示設定を変えても、この件数や検索条件、権限 scope は変わらない
...
- 自社ユーザー名やメールアドレスの保守
- 自社メンバーの追加
- 対象ユーザーを名前・メールアドレスの断片や有効/無効状態で探す
- `適用中` と `検索結果` を見て、現在の絞り込み条件と一覧件数を確認する
...
- 会社画面で `適用中: ...` と `検索結果: N件` が出ている: keyword または状態 filter がかかっている。検索結果件数は現在の filter 後の件数であり、列の表示設定では変わらない
...
- 自社ユーザーを探したい: `ユーザーを探す` で名前・メールアドレスの断片や有効/無効状態を絞り込む
- ユーザー画面で `適用中: ...` と `検索結果: N件` が出ている: keyword または状態 filter がかかっている。検索結果件数は現在の filter 後の件数であり、列の表示設定では変わらない
