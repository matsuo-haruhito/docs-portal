...
`file_uploads` は、受信した単体ファイルを一時ZIPとして staging し、既存の ZIP import dry-run / manifest / `DocumentImporter` の流れへ合流させる。

クライアントPC上のフルパスは `source_path` として参考情報に留め、取り込み上の安全な識別子には `relative_path` を使う。

## 後方互換

このAPI群はまだ利用者がいない前提のため、後方互換は維持しない方針。
旧 `doc_imports` / `zip_imports` ルートは削除対象。
