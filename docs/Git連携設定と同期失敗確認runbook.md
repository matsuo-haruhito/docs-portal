...
履歴が 1 件以上ある状態で、状態や repository の絞り込み結果だけが 0 件になると、`条件に一致するGit同期履歴はありません。` という filtered empty state が出ます。この場合は、適用中の状態や repository 条件を読み、必要なら `絞り込み解除` の button-style action で条件を外して最新 100 件表示へ戻します。
