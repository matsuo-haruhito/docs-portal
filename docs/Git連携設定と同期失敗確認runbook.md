...
- `案件`: 取り込み先 Project
- `リポジトリ`: `owner/repo` 形式の repository 名。GitHub App の installation context が使える場合は候補から選べます
- `ブランチ`: 取り込み対象 branch
...
Git 連携設定が 1 件以上ある場合、一覧の上に `一覧の絞り込み` が表示されます。設定数が多いときは、次の条件で対象を絞ってから設定値や最終同期を見返します。

- `検索`: repository、branch、取込元 path の断片で探す。検索語は空白を正規化し、最大 100 文字まで扱う
- `案件`: 既存の案件 remote search と同じ候補境界で、案件コード・案件名から設定を探す
- `状態`: `すべて`、`有効`、`無効` を切り替える。未知の状態値は all 扱いに戻る
- `表示件数`: 50 件または 100 件で表示する。default は 50 件で、page 移動後も検索語、案件、状態、表示件数を維持する

絞り込み中は `検索結果: N件` と `表示中: X-Y件 / N件` を見て、現在の条件で何件に絞られているかを確認します。`Git連携設定一覧の表示設定` は列の表示・幅だけを変える設定で、絞り込み条件、表示件数、page 範囲、CSV export、Git同期履歴の表示条件は変更しません。

`条件に一致するGit連携設定はありません。` は、登録済み設定はあるが検索語、案件、状態の条件に合う設定がない状態です。この場合は条件を緩めるか、`すべてのGit連携設定を見る` で一覧へ戻します。`まだGit連携は登録されていません。` は設定そのものが 0 件の初回状態なので、まず新規登録 form から同期元を登録します。
...
押下前の confirmation には、案件、repository、branch、取込元パス、状態がラベル付きで表示されます。案件は project code があれば `code / name`、なければ案件名だけを読み、取込元パスが空の場合は root 相当として `/` を読み返します。この confirmation は同期対象の最終確認であり、手動同期 preview、dry-run、GitImportRun の事前作成、credential 検証、履歴 filter 変更を行う画面ではありません。

## 4. `Git同期履歴` 画面で見ること

`Git同期履歴` に履歴がある場合、画面上部の filter で `状態`、`案件`、`リポジトリ`、`ブランチ`、`取込元パス`、`コミット` を絞り込めます。`案件` は案件コード・案件名の remote search で、対象 run の `GitImportSource` に紐づく Project を絞り込みます。`リポジトリ` は `owner/repo` の一部一致、`ブランチ` と `取込元パス` も一部一致で探します。`コミット` は commit SHA の先頭一致で探します。これらの条件は AND 条件で併用され、絞り込み後も表示対象は最新 100 件までです。案件や repository、branch / path を短くしすぎると複数の履歴が混ざり、長くしすぎると 0 件になりやすいため、filtered empty state では適用中の条件を読み返して検索語を短くするか、`絞り込み解除` で最新 100 件表示へ戻します。

`Git同期履歴` の `案件` filter は、run 履歴を案件単位に見返すための条件です。`Git連携` 設定一覧の `案件` filter と同じように案件コード・案件名で探せますが、こちらは保存済み設定の一覧ではなく、作成済み `GitImportRun` の履歴だけを対象にします。案件を選んでも Git import runner、同期対象 repository、branch / path、手動同期の実行範囲、設定一覧の表示条件は変更しません。
...
履歴が 1 件以上ある状態で、状態、案件、repository、branch、取込元パス、commit の絞り込み結果だけが 0 件になると、`条件に一致するGit同期履歴はありません。` という filtered empty state が出ます。この場合は、適用中の状態、案件、各検索条件を読み、必要なら `絞り込み解除` の button-style action で条件を外して最新 100 件表示へ戻します。案件は案件コード・案件名の remote search、repository / branch / source path は一部一致、commit は SHA prefix match なので、`missing-repo` や `deadbeef` のように合わない値や狭すぎる値では 0 件になり、検索語や prefix を短くすると見つかる場合があります。

`絞り込み解除` は表示条件を reset するための導線であり、同期 retry、失敗解消、対象なし保証、GitImportRun の保存内容変更を行う操作ではありません。filtered empty state で 0 件でも、別の案件、状態、repository、branch、path、commit には履歴が残っている可能性があります。
...
- GitHub App の installation context が使える場合、`Git連携` form の `リポジトリ` は候補から選べます。候補取得不可、候補 0 件、installation ID 未設定、token 未設定の場合も `owner/repo` の手入力 fallback は維持します
- 詳細設定は通常開かない管理者・検証向けで、保存済み secret は form に表示されません。空欄保存は既存 secret 維持、新しい値を入力したときだけ更新です
- `Git連携` の設定一覧 filter は、repository / branch / source path の一部一致、案件 remote search、状態、50 / 100 件の bounded 表示を扱います。これは設定一覧の表示範囲だけを変え、登録 form、credential、手動同期、table preferences、Git同期履歴、Git import runner の挙動は変更しません
- `手動同期` confirmation は案件、repository、branch、取込元パス、状態を押下前に読み返す cue であり、手動同期 preview や dry-run ではありません
