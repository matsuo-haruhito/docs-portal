...
- drop で review 候補を作れるのは 1 ファイルだけ。複数ファイルを同時に drop した場合は hidden multipart form を submit せず、`複数ファイルの同時アップロードはまだ未対応です。ZIPにまとめるか、1ファイルずつアップロードしてください。` の alert で止まる
...
upload request が review 候補を作れなかった場合は、版詳細の `アップロード候補の確認` へは進まない。alert を読んで、戻り先の画面で入力や drop 先を見直す。

- 複数ファイルを同時に drop した場合は、request を作らずその場の alert で止まる。複数ファイルを 1 回で扱いたい場合は ZIP にまとめるか、manual upload review flow では 1 ファイルずつ drop する
...
- upload 直後に review 画面へ進まず alert で戻った: alert の内容を読み、案件一覧へ戻った場合は案件を選び直し、案件の文書一覧へ戻った場合はファイル選択や upload 先 path を見直す。複数ファイル drop の alert なら、ZIP にまとめるか 1 ファイルずつ drop し直す
...
- この runbook は current manual upload review flow と、review 候補作成前の alert 付き error fallback だけを扱う
- upload panel の destination 表示は、選択中 path を読みやすく確認するための UI であり、ManualDocumentUpload の保存契約、version 作成、上書き判断を変えるものではない
- manual upload の drop は current support では単一ファイルだけを review 候補にする。複数ファイルを同時に drop しても候補作成や一括 apply は行わず、ZIP import または 1 ファイルずつの manual upload に戻す
