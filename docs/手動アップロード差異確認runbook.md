...
`admin/file_upload_dry_runs` の `単体ファイルアップロードの確認履歴` は、file_uploads API から作成された単体ファイルアップロード dry-run を後から read-only に確認する一覧です。版詳細の upload review flow や `OK` / `NG` を実行する画面ではありません。

- 一覧に出るのは `manual_upload` の dry-run だけ。ZIP / Git連携 / artifact import の dry-run はこの一覧には表示されない
- `dry-run ID` は公開 ID の完全一致検索として読む
- `同期元名・取り込み先パス・内容ハッシュ` は、表示中の safe metadata である `source_name` / `relative_path` / `content_hash` だけを対象にする
- クライアント source path は検索対象外。詳細確認で扱うことはあっても、一覧検索や列表示の安全な metadata と混同しない
- 案件 filter は案件コード・案件名の remote search で探せる。選択済み案件は候補上限外でも復元される
- filter なしで 0 件なら、単体ファイルアップロード確認履歴がまだ作成されていない状態。filter ありで 0 件なら、dry-run ID、同期元名・取り込み先パス・内容ハッシュ、案件、状態を見直すか、`絞り込み解除` で一覧に戻る

この一覧は後追い調査の入口であり、manual upload API の契約変更、client source path の検索対象化、ZIP / Git連携 / artifact import dry-run の統合、dry-run apply / confirm の新設は含めない。
