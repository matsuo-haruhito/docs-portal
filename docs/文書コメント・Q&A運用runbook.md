...
- active / future handoff: `#3208` は未解決 Q&A / 確認事項を read-only handoff しやすくする feature queue です。dashboard の `受付中Q&A候補` より広い workspace 文脈、internal / external の copyable summary、`comment_q` / `comment_tab` / `comment_author_id` を含む handoff は current support として扱いません。
