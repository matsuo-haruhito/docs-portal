...
このメモは、`生成ファイル実行履歴` detail の下部に出る診断ブロックを読むときの補助です。基本の切り分け順、filter、retry の操作境界は [生成ファイル再試行と定期ジョブ管理 runbook](./生成ファイル再試行と定期ジョブ管理runbook.md) を正本にします。
...
- 長い metadata JSON や error log 全文をそのまま検索欄へ貼るのではなく、`gfr...` の一部、入力パスや生成パスの特徴語、短い error 断片、metadata に残る event public ID など 100 文字以内の手掛かりで探します。
