...
## evidence family の3区分

visual evidence が必要な PR では、まず対象が次のどれかを分けます。同じ screenshot や source inspection でも、区分が違うと完了条件と限界が変わります。

| 区分 | 主な対象 | 最低限残す evidence | source inspection だけで完了にしない条件 |
| --- | --- | --- | --- |
| upstream static docs / mockup / visual reference | `rails_fields_kit` の `doc/*sample_evidence.html` / `doc/*visual_reference*.html`、`tree_view-rails` の mockup gallery、`rails_table_preferences` の generated demo | nonblank、主要 heading / sample region、desktop / narrow の粗い崩れ確認。先行 pattern として `tree_view-rails#854` の Playwright screenshot smoke を参照する | PR の acceptance が hover / focus / keyboard /実ブラウザ layout を含む場合 |
| docs-portal runtime host app UI | Markdown preview、site viewer iframe、admin list / form、viewer tree など current app 画面 | normal page / embedded iframe / representative screen の browser-capable evidence、確認 viewport、未確認 surface | source spec が text / class / ARIA だけを守り、layout / readability / iframe styling を acceptance に含む場合 |
| PR reviewer evidence | review follow-up comment、PR body、manual spot check record | screenshot / browser smoke / source inspection / limits / follow-up を1セットで残す | CI green だけで visual acceptance を満たしたと誤読される場合 |

この3区分は evidence の置き場所を決めるためのものです。static artifact の先行 smoke を host app runtime evidence の代替にしたり、docs-portal の runtime cue を upstream gem の current support として書いたりしません。
...
| --- | --- | --- | --- | --- |
| `tree_view-rails` | `docs/mockups/`、persisted state guide 用 HTML / CSS | static artifact は link / heading / nonblank / sample region の lightweight check。browser smoke は mockup が実操作を持つ場合だけ upstream issue で扱う。`tree_view-rails#854` は representative viewport の Playwright smoke 先行 pattern として参照する | `#903` では sidebar tree、detail tree、persisted state の representative smoke と rollback note を残す | downstream 採用前の JS hook 追加、全 repo 一律 Playwright 化、screenshot baseline |
