...
Normal source builds and recovery after a previously successful artifact becomes missing or inconsistent use separate bounded retry cycles. Reconciliation changes the persisted preview build reason to artifact recovery and resets the current cycle's attempt count only on the first transition from a normal source build. Artifact recovery then uses the same five-attempt ceiling and persisted backoff. The recovery reason and consumed attempt count remain after success, so a later artifact loss continues with the remaining recovery budget instead of opening an unlimited new cycle. An abandoned artifact-recovery cycle is not rebuilt automatically, but reconciliation still repairs its database state when a valid installed artifact reappears. For legacy or normal-build abandoned rows that retain a site path while the artifact is missing, reconciliation opens the single bounded artifact-recovery cycle; the persisted reason prevents that compatibility recovery from being opened again.

When generated HTML is unavailable and an embeddable source Markdown file exists, the document viewer remains readable through raw Markdown fallback. The parent viewer must display that fallback and its persisted preview state explicitly; successful fallback rendering must not hide an unavailable, retrying, recovering, or abandoned generated preview.
