...
3. 対象案件に Office ファイルを持つ文書版を用意する
4. 一覧の `preview利用` 列で `previewで使用中` の行を確認する
5. 文書詳細や版詳細から Office ファイルの inline preview を開く
6. Graph preview が開くことを確認する
7. 必要なら、Graph 接続を無効化した状態または Graph 側失敗時に Google Drive fallback が成立するか確認する
