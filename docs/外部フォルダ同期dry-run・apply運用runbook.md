...
1. 対象の同期元が存在するか、一時停止されていないかを一覧で確認する
2. `Google OAuthが未接続` の案内が出ているなら、まず接続を済ませる
3. 初回取り込みや変更確認では、先に `同期プレビュー` を実行する
4. warning や error が出たら、詳細画面の `同期履歴` と `結果詳細` を見る
5. 継続運用で自動検知まで使うなら、`バックグラウンド同期` と `変更通知の購読` を確認する
...
- `同期設定名`: 詳細画面への入口
- `連携先`: current main では Google Drive
- `Google DriveフォルダID`: 対象フォルダ ID と表示用 path
- `同期状態`: 有効 / 無効
- `最終同期日時`: 直近の同期時刻
- `最新安全判定`: 直近 run の安全判定
...
- current provider は `google_drive` のみです
...
この runbook は current main の Google Drive 運用に限定しており、将来対応の provider-aware UI や Graph 同期の仕様は追加しません。
