...
| SharePoint / OneDrive (`microsoft_graph` source) | `Microsoft Graph接続` を前提に共有 URL を保存し、`drive_id` / `folder_item_id` / `folder_path` / `site_id` などの metadata を詳細画面で確認できます。 | Graph -> Portal の pull 型同期本体、`dry_run` / `apply` / `enqueue` / 変更通知は `#1030` 以降の範囲です。現時点では同期操作へ進めません。 |
