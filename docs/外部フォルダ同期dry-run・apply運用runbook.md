...
まだ `同期履歴` が 0 件のときは、同期プレビュー / 同期がまだ実行されていない状態として読みます。Google Drive source なら初回 `同期プレビュー` へ戻り、SharePoint / OneDrive source なら metadata-only source のため履歴 0 件を同期成功や失敗なしの証跡として扱わないでください。
...
Google Drive source で `変更通知の購読` が 0 件のときは、購読未開始または購読なしとして読みます。0 件は購読済み、正常購読中、失敗なしの証跡ではありません。変更通知まで使う運用なら `変更通知の購読を開始` の位置へ戻り、期限や直近エラーは購読が作成された後にこの card で確認します。
...
`同期アイテム` が 0 件のときは、同期プレビュー前、同期対象なし、またはまだ対応付けが作られていない状態のいずれかとして読みます。0 件だけで同期済み、失敗なし、外部フォルダが空、と断定しないでください。Google Drive source では上の `同期履歴` の `結果詳細` と初回 `同期プレビュー` へ戻り、SharePoint / OneDrive source では同期本体が current support 外であることを前提に metadata 確認へ戻します。
