...
README からこの runbook へ入ったときは、まず次の表で provider ごとの current support と未対応範囲を切り分けます。外部同期に見える項目でも、Git import は `Git連携` / `Git同期履歴` の別画面、Google Drive は同期本体の運用対象、SharePoint / OneDrive は metadata 保存 first slice として扱います。

| provider / 入口 | current support | 未対応・次に見る issue |
| --- | --- | --- |
...
| SharePoint / OneDrive (`microsoft_graph` source) | `Microsoft Graph接続` を前提に共有 URL を保存し、`drive_id` / `folder_item_id` / `folder_path` / `site_id` などの metadata を詳細画面で確認できます。SharePoint webhook の GET / POST 受信 route は実装準備として存在し、validation token 応答と通知 payload の記録までは controller が受け付けます。 | Graph -> Portal の pull 型同期本体、`dry_run` / `apply` / `enqueue`、Graph subscription 作成、SharePoint / OneDrive 変更通知を起点にした同期運用は `#1030` 以降の範囲です。受信 route があるだけでは同期操作へ進めません。 |
...
4. 新規登録する場合は、先頭 card の `Google Drive から始める` と `SharePoint / OneDrive を準備する` のどちらに当たるかを先に切り分ける
...
10. SharePoint / OneDrive で webhook 受信の記録だけを確認する場合は、同期操作ではなく「受信イベントの保存有無」と「matching source / verification の成否」を見る。受信 route の存在をもって SharePoint / OneDrive 同期が運用可能とは扱わない
...
- webhook 受信口は `external_folder_sync_webhooks/sharepoint` の GET / POST として存在し、Microsoft Graph の validation token 応答と通知 payload 記録の準備だけを担います
- `dry_run` / `apply` / `enqueue` / Graph subscription 作成 / 変更通知起点の同期運用は後続 issue の範囲なので、この runbook では保存済み metadata と受信準備の境界確認までを current support として扱います
...
- webhook 受信イベントが見える場合でも、まず subscription と verification が matching source に紐づいたか、payload が記録されたかを確認対象にする
- 詳細画面の案内どおり、差分同期本体と変更通知起点の同期運用は後続 issue 範囲として切り分ける
...
SharePoint / OneDrive source は first slice として metadata 保存と webhook 受信準備の境界確認までなので、この節の操作説明は Google Drive source を前提に読みます。SharePoint / OneDrive source では `dry_run` / `apply` / `enqueue` / 変更通知起点の同期に進まず、接続と保存済み metadata、受信イベントがある場合は verification / payload 記録の確認を優先してください。
...
SharePoint / OneDrive の通知 payload が記録されている場合でも、current `main` の同期 runner は Google Drive のみを support しています。SharePoint / OneDrive では validation token 応答、`subscriptionId` / `clientState` による照合、payload 保存までを受信準備として読み、Graph -> Portal 同期本体や Graph subscription 作成が運用可能になったとは扱いません。matching source がない、または verification が通らない event は、同期に進める対象ではなく受信設定の見直し対象です。
...
- SharePoint / OneDrive webhook は `external_folder_sync_webhooks/sharepoint` の GET / POST 受信口として存在し、validation token があれば plain text で返し、通知 payload は `value` 配列単位で受信イベントとして記録します
- SharePoint / OneDrive webhook event は `subscriptionId` と `clientState` で active subscription / verification を照合します。ただし、Graph subscription 作成 UI / 運用、Graph -> Portal の pull 型同期本体、SharePoint / OneDrive 向け runner は current support 外です
- SharePoint / OneDrive を同期元として `dry_run` / `apply` / `enqueue` / 変更通知起点の同期まで進める運用は未対応です
