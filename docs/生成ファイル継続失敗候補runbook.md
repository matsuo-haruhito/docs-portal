...
- `Markdown digest preview` は候補の引き継ぎ用であり、通知送信や ack / escalation の記録ではない。
...
5. 通知前の dry-run や admin 内の引き継ぎが必要な場合は `Markdown digest preview` または `GeneratedFiles::RunFailureAlertHandoff` の payload を確認する
