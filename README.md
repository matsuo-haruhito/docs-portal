...
- HTML 表示は Document 詳細から辿れても、実際の配信 URL は Project code を使う `/projects/:code/site/...` に寄せます。
- 公開側の主要 route は `Project: code`、`Document: slug`、`DocumentVersion / DocumentFile: public_id` です。seed 用サンプルや外部導線を確認するときも numeric id ではなく、これらの公開識別子を前提にします。
