---
slug: /client-file-upload-api
---

# Client file upload API

同期クライアントや簡易アップローダーは、単体ファイルを `POST /api/internal/file_uploads` に送ります。
アップロードと公開は分離し、必ず dry-run を作ってから本実行します。
...
`source_path` は監査・表示用であり、manifest の branch metadata には使いません。
サーバー側の取り込み識別には `relative_path` と、採用後の `source_commit_hash` を使います。
...
成功時は `PublishJob` のIDと状態を返します。本実行レスポンスは公開反映結果だけを返し、dry-run 用の `file_upload_preview` は返しません。
...
`file_uploads` で作った dry-run は `file_uploads` で本実行します。
`zip_uploads` で作った dry-run は `zip_uploads` で本実行します。
別APIの `import_dry_run_id` を渡しても実行しません。

`file` と `import_dry_run_id` を同時に送った場合は、`file` を優先して新しい dry-run を作ります。
既存 dry-run を本実行するときは `import_dry_run_id` だけを送ってください。
...
`docs\\README.md` のような Windows 区切りの相対パスは `docs/README.md` に正規化します。
