...
| パラメータ | 必須 | 内容 |
| --- | --- | --- |
| `artifact_root` | 必須 | 取り込み対象成果物のルートディレクトリ |
| `manifest_path` | 必須 | publish manifest のパス |
...
curl -X POST https://portal.example.com/api/internal/doc_imports \
  ...
  -F "artifact_root=/path/to/artifact" \
  -F "manifest_path=/path/to/artifact/publish.json" \
...
| `source_repo` | 任意 | 元リポジトリ |
| `source_branch` | 任意 | 元ブランチ |
| `source_commit_hash` | 任意 | 元コミット |
| `version_label` | 任意 | 作成する文書版のラベル |
| `status` | 任意 | 作成する文書版の公開状態 |
