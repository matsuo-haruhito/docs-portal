...
GitHub Actions 専用APIではなく、Git push、artifact import、file upload、ZIP upload、Git pull 型同期、Webhook、Microsoft Graph などの更新・連携フローを扱う管理者向け仕様です。
...
- レスポンス形式: JSON
- artifact 取り込み対象パス: `storage/imports/` 配下のみ
...
このAPIは、ファイル実体をアップロードする入口ではなく、サーバー上に配置済みの artifact root と publish manifest を指定する入口です。
...
```bash
curl -X POST https://portal.example.com/api/internal/artifact_imports \
...
ZIP アップロードは dry-run を必須とし、本実行時には dry-run の成果物を再利用します。
...
```bash
curl -X POST https://portal.example.com/api/internal/zip_uploads \
  -H "Authorization: Bearer ${DOC_IMPORT_TOKEN}" \
...
現行実装で Microsoft Graph API 呼び出しに使う必須値は **Drive ID** です。Rails は `drive_id` を使って、preview 用フォルダへのアップロードと、アップロードした driveItem の `/preview` を実行します。

`Site ID` は現行実装では API 呼び出しに直接使っていません。管理者が「この Drive ID はどの SharePoint site / document library のものか」を後から追跡するための任意メモとして扱います。

```text
Site ID: 任意。例: contoso.sharepoint.com,site-id,web-id
Drive ID: 必須。例: b!xxxxxxxxxxxxxxxx
```
...
具体的には、次の2種類の Graph 操作が必要です。

- `PUT /drives/{drive_id}/root:/{preview_folder_path}/{file_name}:/content`
  - preview 用 Drive へのファイルアップロード
- `POST /drives/{drive_id}/items/{item_id}/preview`
  - アップロードした driveItem の一時 preview URL 取得

Microsoft Entra の **API permissions** では、Microsoft Graph の **Application permissions** として次を追加し、管理者同意を付与します。

| Permission | 種別 | 用途 | 備考 |
| --- | --- | --- | --- |
| `Sites.ReadWrite.All` | Application | SharePoint / OneDrive for Business の drive に preview 用ファイルをアップロードし、preview URL を取得する | 標準推奨。管理者同意が必要 |

`/preview` だけなら application permission の least privileged は `Files.Read.All` ですが、このポータルは preview 前に Drive へ一時アップロードするため、読み取り専用では不足します。
アップロードも行う運用では `Sites.ReadWrite.All` を基本設定とします。

テナント全体の `Sites.ReadWrite.All` を避けたい場合は、`Sites.Selected` など site 限定の権限設計を検討できます。ただし、その場合は Entra の権限付与に加えて、対象 SharePoint site への個別 grant が必要になります。現行仕様ではまず `Sites.ReadWrite.All` を標準手順として扱います。
...
| Site ID | `contoso.sharepoint.com,...` | 任意。Drive ID の取得元 SharePoint site を追跡するための運用メモ。現行の API 呼び出しでは未使用 |
| Drive ID | `b!xxxxxxxx` | preview 用ファイルを置く Drive ID。Office preview 実行に必須 |
...
3. Microsoft Graph の Application permission として `Sites.ReadWrite.All` を追加し、管理者同意を付与する。
4. SharePoint / OneDrive の preview 用 Drive と folder を用意する。
5. Drive ID を取得する。Site ID は必要に応じて管理用メモとして控える。
6. 管理メニューの **Microsoft Graph** で案件ごとに接続を登録する。
7. 利用者が Office ファイルを embedded viewer で開く。
8. Rails が対象ファイルを preview 用 folder へ一時アップロードし、Graph の driveItem preview URL を取得する。
9. iframe 内で Graph preview URL へ redirect する。
