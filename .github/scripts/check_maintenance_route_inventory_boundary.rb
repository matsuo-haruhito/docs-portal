...
MAINTENANCE_ROUTE_INVENTORY_DOC_SIGNALS = [
  ...
  "| 外部フォルダ同期 source設定 | `admin/external_folder_sync_sources#create` / `#update` / `#destroy` | current。",
  "| 外部フォルダ同期実行系 | `admin/external_folder_sync_sources#dry_run` / `#apply` / `#force_apply` / `#enqueue` / `#subscribe` / `#unsubscribe` / `#recheck_metadata` | 要判断。",
