...
MAINTENANCE_ROUTE_INVENTORY_DOC_SIGNALS = [
  ...
  "| 生成ファイルイベントdispatch / preview reconciliation | `GeneratedFileEventDispatchJob`, `DocusaurusPreviewBuildReconciliationJob` | current。`READ_ONLY_MAINTENANCE` 中はevent dispatch・stale processing回収、preview artifact修復・再enqueue・reconciled時刻更新を行わず、保存済み状態の確認だけを残す。",
  ...
  "| Webhook 手動・自動再送 | `admin/webhook_deliveries#retry_dispatch` / `#retry_failed`, `WebhookDeliveryAutoRetryJob` | current。停止中は手動再送に加え、自動再送のclaim・stale claim回収・HTTP POSTを行わない。送信履歴、retry状態、failure handoffはread-onlyに残す。",
