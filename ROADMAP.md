...
| `rails_fields_kit` フォーム | `admin/document_sets` が host app 側の実装済み代表例。対象文書は title / slug の RFK remote picker、既存 table row への移動・絞り込み、固定版 selector を current support として持つ。`admin/document_usage_reports` と `admin/read_confirmations` では案件選択の `rfk_select` を利用済み | #1348 / PR #1366 は document set の local filter / fixed version selector。#1985 の文書 remote picker first slice は PR #2079 で clean branch から merge 済み | document version remote search、table replacement、project / company / user selection、他フォームへの横展開は別 issue に分ける。案件選択の `rfk_select`、文書セット対象文書 remote picker、document version remote search を同じ current support として混ぜない |
...
特に件数が増えやすい選択欄は、Tom Select と remote search を前提に UX を改善する。current main では `admin/document_sets` の対象文書で、文書名 / URL 識別子を使う remote picker、既存 table row への移動・絞り込み、local filter、選択済みのみ表示、fixed version selector を組み合わせて使う。`admin/document_usage_reports` と `admin/read_confirmations` では案件選択で `rfk_select` を使う。これらは current support だが、document version remote search、project / company / user selection、upstream RFK API 変更、pinned ref bump までは含めない。

候補:

- project / company / user の選択
- document_version の選択
- 外部同期元や権限設定の関連レコード選択
