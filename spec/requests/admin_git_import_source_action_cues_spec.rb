...
RSpec.describe "Admin git import source action cues", type: :request do
  ...
  it "separates manual sync, edit, and delete actions for enabled and disabled sources" do
    ...
    expect(response.body).to include(
      "Git連携設定の手動同期を今すぐ実行します。設定の編集や削除は行いません。案件: GIT001 / Main Docs、リポジトリ: example/active-docs、ブランチ: release/main、取込元パス: docs/current、状態: 有効"
    )
    expect(response.body).to include(
      "Git連携設定の手動同期を今すぐ実行します。設定の編集や削除は行いません。案件: GIT002 / Archive Docs、リポジトリ: example/disabled-docs、ブランチ: release/archive、取込元パス: docs/archive、状態: 無効"
    )
    expect(response.body).to include(
      "Git連携設定を削除します。手動同期ではなく、設定を削除する操作です。案件: GIT001 / Main Docs、リポジトリ: example/active-docs、ブランチ: release/main、取込元パス: docs/current、状態: 有効"
    )
    expect(response.body).to include(
      "Git連携設定を削除します。手動同期ではなく、設定を削除する操作です。案件: GIT002 / Archive Docs、リポジトリ: example/disabled-docs、ブランチ: release/archive、取込元パス: docs/archive、状態: 無効"
    )
  end
end
