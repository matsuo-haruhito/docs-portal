...
RSpec.describe "API internal file upload content hash", type: :request do
  ...
  it "uses content_hash when source_commit_hash is omitted" do
    uploaded_file = build_uploaded_file("# Content Hash\n")
    ...
    expect(response.parsed_body.dig("file_upload_preview", "source_commit_hash")).to eq("content-hash-123")

    dry_run = ImportDryRun.find_by!(public_id: response.parsed_body.fetch("dry_run_id"))
    expect(dry_run.source_commit_hash).to eq("content-hash-123")
  ensure
    uploaded_file&.tempfile&.close!
  end

  it "prefers source_commit_hash over content_hash" do
