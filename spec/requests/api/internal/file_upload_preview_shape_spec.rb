...
RSpec.describe "API internal file upload preview shape", type: :request do
  let(:token) { "secret-token" }
  let(:headers) { { "Authorization" => "Bearer #{token}" } }
  let(:project) { create(:project, code: "FILESHAPE", name: "File Shape Project") }
  ...
  it "returns the expected preview keys" do
    file_content = "# Preview Shape\n"
    ...
    expect(preview.keys).to contain_exactly(
      "source_name",
      "relative_path",
      "source_path",
      "file_size",
  ...
  def build_uploaded_file(content)
    tempfile = Tempfile.new(["file-upload-preview-shape", ".md"])
    tempfile.binmode
