...
RSpec.describe "API internal file upload validation", type: :request do
  let(:token) { "secret-token" }
  let(:headers) { { "Authorization" => "Bearer #{token}" } }
  let(:project) { create(:project, code: "FILEVALID", name: "File Validation Project") }
  ...
  it "rejects non-multipart file values" do
    ...
    expect(response.parsed_body["error"]).to include("multipart upload")
  end
end
