...
RSpec.describe "Document review comments", type: :request do
  ...
  it "keeps review comment decision notices and comment redirect context" do
    ...
    follow_redirect!
    expect(response.body).to include("レビューコメントを解決済みにしました。")
    ...
    follow_redirect!
    expect(response.body).to include("レビューコメントを却下扱いにしました。")
    expect(response.body).to include("検索条件: #{normalized_query}")
    expect(response.body).not_to include(long_query)
  end
  ...
  it "counts and filters unresolved Q&A threads by open status" do
    ...
    expect(qa_panel_text).to include("Q&A状態: 受付中")
    expect(qa_panel_text).to include("Q&A状態: 回答済み")
    expect(qa_panel_text).to include("Q&A状態: クローズ")
  ...
  it "keeps Q&A decision notices and safe redirect context across document and version routes" do
    ...
    expect(response.body).to include("検索条件: reuse")
    ...
    expect(response.body).to include("検索条件: visible")
  end
