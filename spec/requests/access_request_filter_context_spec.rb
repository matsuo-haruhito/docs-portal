...
RSpec.describe "Access request filter context", type: :request do
  ...
  it "shows active filter context when combined filters return no rows" do
    ...
    expect(page_text).to include("表示中条件: 状態: 承認済み / 検索: missing target / 要求権限: ダウンロード / 対象種別: ファイル")
    expect(page_text).to include("状態: 承認済み / 検索: missing target / 要求権限: ダウンロード / 対象種別: ファイル に一致するアクセス申請はありません。")
  ...
  it "keeps pending cancel guidance focused on the row and current filters" do
    ...
    get access_requests_path(status: :pending, requested_access_level: :download, requestable_type: "DocumentFile")
