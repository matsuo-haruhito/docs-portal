...
RSpec.describe "Admin access log AI context filter guidance", type: :request do
  ...
  it "explains that AI context mode and scope only apply to AI context exports" do
    ...
    expect(guidance_group.text.squish).to include("AI出力モード・範囲は page / file / zip / webhook など他の対象種別では条件から外れ")
    expect(guidance_group.text.squish).to include("CSV export や有効な条件にも残りません。")
