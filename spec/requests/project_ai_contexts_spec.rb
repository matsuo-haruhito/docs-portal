...
RSpec.describe "Project AI contexts", type: :request do
  ...
  it "keeps selected document ids across preview and scoped exports" do
    ...
    get project_ai_context_path(project, mode: :full, document_q: "selected", document_ids: [internal.id], candidate_view: :selected)
    expect(response).to have_http_status(:ok)
    ...
    expect(ai_context_link_href("検索候補へ戻る")).to include("mode=full", "document_q=selected", "document_ids%5B%5D=#{internal.id}")
