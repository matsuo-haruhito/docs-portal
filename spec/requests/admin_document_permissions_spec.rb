...
RSpec.describe "Admin document permissions", type: :request do
  ...
  def select_option_texts(field_name)
    parsed_html.css(%(select[name="#{field_name}"] option)).map { _1.text.squish }.reject(&:empty?)
  end
  ...
  it "shows empty-state guidance when no document permissions exist" do
    ...
    expect(select_option_texts("document_permission[company_id]")).to include("会社向けに付与する場合に選択")
    expect(select_option_texts("document_permission[user_id]")).to include("ユーザー向けに付与する場合に選択")
