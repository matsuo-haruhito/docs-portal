...
RSpec.describe "Admin access request action copy", type: :request do
  ...
  it "keeps approval and rejection cues close to pending row actions without changing action params" do
    ...
    expect(page_text).to include("承認: 現行の付与先に要求権限を反映します。")
    expect(page_text).to include("却下: 権限は変更せず、理由を残して申請を閉じます。")
    expect(page_text).to include("定型候補と補足は却下理由の入力補助です。承認基準や権限付与仕様はここでは変更しません。")
    ...
    expect(download_row.text.squish).to include("承認: 現行の付与先に要求権限を反映します。")
