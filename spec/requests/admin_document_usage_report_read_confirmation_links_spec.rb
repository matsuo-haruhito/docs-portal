...
RSpec.describe "Admin document usage report read confirmation links", type: :request do
  ...
  it "does not add period params when no period is selected" do
    ...
    expect(summary_params).to eq("project_id" => project.id.to_s)
    expect(row_params).to eq("project_id" => project.id.to_s, "document_slug" => document.slug)
  end

  it "does not leak invalid dates or unrelated usage filters to read confirmation links" do
    get admin_document_usage_reports_path(
      project_id: project.id,
      q: "manual",
      usage_filter: "used",
