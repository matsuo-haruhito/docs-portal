...
RSpec.describe "Admin access log filter form", type: :request do
  ...
  it "shows query targets and length cues for free-text access log filters" do
    ...
    expect(parsed_html.at_css(".filter-toolbar.admin-filter-toolbar")).to be_present
    expect(parsed_html.at_css(%(.filter-toolbar__field--enum select[name="action_type"]))).to be_present
    expect(parsed_html.at_css(%(.filter-toolbar__field--enum select[name="target_type"]))).to be_present
    expect(parsed_html.at_css(%(.filter-toolbar__field--remote select[name="project_id"]))).to be_present
    expect(advanced_filters.at_css(%(.filter-toolbar__field--remote select[name="company_id"]))).to be_present
    expect(advanced_filters.at_css(%(.filter-toolbar__field--remote select[name="user_id"]))).to be_present
    expect(advanced_filters.at_css(%(.filter-toolbar__field--query input[name="q"]))).to be_present
    expect(advanced_filters.at_css(%(.filter-toolbar__field--query input[name="document_q"]))).to be_present
    expect(advanced_filters.css(%(.filter-toolbar__field--date input[type="date"])).size).to eq(2)
    expect(page_text).to include("対象名・IPアドレス、文書名・URL識別子は最大100文字です。")
