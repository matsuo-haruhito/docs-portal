  ...
  describe ".portal_visible_to" do
    it "matches the current external portal visibility predicate for publication windows" do
      travel_to Time.zone.local(2026, 1, 15, 12, 0, 0) do
        ...
        expect(described_class.portal_visible_to(external_user)).to match_array(expected_documents)
        expect(described_class.portal_visible_to(external_user)).to include(no_version_document, active_document)
        expect(described_class.portal_visible_to(external_user)).not_to include(
          draft_document,
          future_document,
          expired_document,
