...
RSpec.describe "site viewer iframe height source" do
  ...
  it "marks the site viewer iframe for auto height sync" do
    expect(view_source).to include('data-docs-portal-auto-height="true"'.delete("\\"))
  end
  ...
  it "keeps the embedded response hook minimal but reactive to later height changes" do
    aggregate_failures do
