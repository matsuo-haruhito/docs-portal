...
RSpec.describe "frontend initialization inventory source" do
  ...
  let(:interaction_policy_source) { read_source(".kiro/steering/frontend-interaction-policy.md") }
  let(:roadmap_source) { read_source("ROADMAP.md") }

  let(:direct_entrypoint_setup_patterns) do
    {
      "querySelectorAll" => "Move DOM scanning into a Stimulus controller or dedicated module instead of app/frontend/entrypoints/application.ts.",
  ...
  it "keeps the Vite entrypoint limited to imports and Stimulus controller registration" do
    ...
    aggregate_failures do
      ...
      direct_entrypoint_setup_patterns.each do |pattern, failure_message|
        expect(top_level_source).not_to include(pattern), failure_message
      end
    end
  end
  ...
  it "keeps the inventory and policy aligned with the entrypoint guard" do
    aggregate_failures do
      expect(inventory_source).to include("`app/frontend/entrypoints/application.ts`")
      ...
      expect(interaction_policy_source).to include("`application.ts` に直接 `querySelectorAll` とイベント登録を増やさない")
