...
RSpec.describe "manual-document-upload controller source" do
  let(:source_path) { File.expand_path("../../app/frontend/controllers/manual_document_upload_controller.ts", __dir__) }
  ...
  it "keeps iframe document drag listeners attachable and removable" do
    ...
    expect(source).to include("const frameDocument = this.frameDocument()")
  ...
  it "keeps multiple file drops as inline preview without browser alert or submit" do
    ...
    expect(source).to include("this.showMultiFilePreview(files)")
