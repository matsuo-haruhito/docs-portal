...
RSpec.describe "admin external integration project selectors source" do
  ...
  it "keeps provider-aware guidance for the external folder sync source detail" do
    aggregate_failures do
      ...
      expect(external_folder_sync_source_show).to include("h2 SharePoint / OneDrive 変更通知の受信イベント")
      expect(external_folder_sync_source_show).to include("受信済み webhook event の確認用表示")
