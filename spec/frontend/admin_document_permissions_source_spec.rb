...
RSpec.describe "admin document permissions source" do
  ...
  it "wires both admin document permission tables through rails table preferences" do
    aggregate_failures do
      ...
      expect(index_source).to include('render FilterToolbarComponent.new(label: "文書権限の検索条件")')
      expect(index_source).to include("toolbar.with_field(size: :remote)")
      ...
      expect(index_source).to include("render ListFooterComponent.new(pagination: pagination")
  ...
  it "keeps option builders and column metadata in the helper" do
    aggregate_failures do
      ...
      expect(helper_source).to include('table_preferences_column(:download_allowed, label: "ダウンロード", default_width: 115')
      expect(helper_source).to include("def document_permissions_table_columns")
      expect(helper_source).to include('table_preferences_column(:actions, label: "操作", default_width: 100')
