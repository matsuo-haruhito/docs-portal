...
RSpec.describe ZipImportDocumentScanner do
  ...
  before do
    ...
    File.write(root.join("README.md"), <<~MARKDOWN)
      ...
      [Matrix](matrix.xlsx)
    MARKDOWN
    File.write(root.join("policy.pdf"), "pdf")
    File.write(root.join("matrix.xlsx"), "excel")
  ...
  it "detects candidates according to policy, and separates orphan and skipped files" do
    result = scanner.call

    expect(result.documents.map(&:logical_path)).to contain_exactly("README.md", "guides/system.mmd")
    expect(result.documents.find { _1.logical_path == "README.md" }.attachment_paths.map { scanner.send(:logical_path_for, _1) }).to include(
      "README.md",
      "guides/flow.png",
      "matrix.xlsx",
      "policy.pdf"
    ...
    file_backed_result = described_class.new(root:, candidate_policy: :file_backed_documents).call
    readme = file_backed_result.documents.find { _1.logical_path == "README.md" }

    expect(file_backed_result.documents.map(&:logical_path)).to contain_exactly(
      "README.md",
      "guides/system.mmd",
      "matrix.xlsx",
      "policy.pdf"
    )
    expect(readme.attachment_paths.map { scanner.send(:logical_path_for, _1) }).to contain_exactly("README.md", "guides/flow.png")
  end
end
