...
| `app/services/generated_files/file_change_event_registry.rb` | ファイルCRUDイベントruleを読み込むRegistry |
| `app/services/generated_files/generated_job_registry.rb` | 生成Job定義を読み込み、変更ファイル/JobIDから対象Jobを選ぶRegistry |
| `app/services/generated_files/change_event_handler.rb` | CRUDイベントを受け取り、`config/file_change_event_jobs.yml` に従ってJobをenqueueまたはbufferする共通ハンドラ |
| `app/services/generated_files/event_buffer.rb` | 短時間に連続したファイルCRUDイベントを `generated_file_events` に集約するbuffer |
| `app/jobs/generated_file_event_dispatch_job.rb` | 期日到来した `generated_file_events` をまとめて `GeneratedFileChangeEventJob` へ再投入するJob |
...
| `app/models/generated_file_event.rb` | debounce / coalesce 用のファイル変更イベントモデル |
...
`params.debounce_seconds` を指定したruleは、すぐに `GeneratedFileJob` を起動せず、まず `GeneratedFiles::EventBuffer` が `generated_file_events` にpendingイベントとして保存します。
...
GeneratedFiles::EventBuffer
  ↓
generated_file_events pending
  ↓ scheduled_at 到来
GeneratedFileEventDispatchJob
  ↓
GeneratedFileChangeEventJob
  ↓ debounce済みmetadata付きなので再bufferしない
GeneratedFileJob
```

同じ `path / operation / event_source` のpendingイベントは1件にまとめ、`occurrences_count` を増やして `scheduled_at` を後ろへ延ばします。これにより、短時間に同じファイルが何度も保存・同期・アップロードされても、最後に1回だけ生成Jobが実行されます。

`GeneratedFileEventDispatchJob` から再投入されるイベントには、以下のmetadataが付与されます。

| metadata | 内容 |
|---|---|
| `generated_file_event_public_ids` | 集約元イベントID |
| `generated_file_event_occurrences_count` | 集約された発生回数 |

このmetadataがある場合、`ChangeEventHandler` は `debounce_seconds` が設定されていても再度bufferせず、実Jobへ進めます。
...
| `GeneratedFileEventDispatchJob` | 固定キー `generated-file-event-dispatch` |
| `GeneratedFileJob` | `job_ids` があればJobID、なければ変更ファイル一覧 |

同時実行抑制はSolid Queueのconcurrencyで行い、短時間連続イベントの集約は `generated_file_events` と `GeneratedFileEventDispatchJob` で行います。
